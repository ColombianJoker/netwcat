...
// ... rest of the code ...
  ...
  fprintf(stream, "  Server mode: " PROG_NAME " -l PORT [-o FILE] [-w WRITELIMIT] "
                  "[-r READLIMIT] [-v]\n");
  ...
  fprintf(stream, "    -r LIMIT      Stop reading after LIMIT bytes\n");
  fprintf(stream, "    -w LIMIT      Stop writing after LIMIT bytes\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "i:o:r:w:l:c:hv")) != -1) {
    ...
    case 'r': read_limit = strtoull(optarg, NULL, 10); break;
    case 'w': write_limit = strtoull(optarg, NULL, 10); break;
    case 'l': listen_port = optarg; break;
    case 'c': connect_hostport = optarg; break;
    case 'v': verbose = 1; break;
    case 'h': print_usage_and_exit(EXIT_SUCCESS); break;
    default: print_usage_and_exit(EXIT_FAILURE);
    }
  }
  ...
  if (listen_port && infile) {
    fprintf(stderr, PROG_NAME ": error: cannot use -i with -l (server mode only receives)\n");
    fprintf(stderr, "©️ 2026, Ramón Barrios Láscar.\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (listen_port) {
    ...
    if (verbose) {
      ...
      if (write_limit > 0) {
        fprintf(stderr, PROG_NAME ": listening on %s:%s and writing up to %llu bytes to %s\n",
    ...
    // Use sockaddr_storage to ensure we have enough memory to hold an IPv6 address
    ...
    if (verbose) {
      ...
      // getnameinfo converts the raw binary address into readable strings for both IPv4 and IPv6
      if (getnameinfo((struct sockaddr *)&client_addr, client_len, 
        ...
        fprintf(stderr, PROG_NAME ": connection from unknown client received\n");
      }
    }
...
loop_end:
  ...
  if (verbose) {
    if (listen_port) {
      fprintf(stderr,
              PROG_NAME ": received from port %s and written %llu bytes to %s\n",
